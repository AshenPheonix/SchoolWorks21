...
bool wordIsPalindrome (string word){
    if(word.length()<=0)
        ...
        return (true && wordIsPalindrome(word.substr(1,word.length()-2)));
    else
        return false;
}
...
string arrayForwardsAsString (int array[], unsigned int startIndex, unsigned int size){
    ...
    if(startIndex>=size)
        ...
        temp << array[startIndex] << " " << arrayForwardsAsString(array, startIndex+1, size);
    return temp.str();
}
...
string arrayBackwardsAsString (int array[], int startIndex, unsigned int size){
    ...
    if(startIndex<0)
        ...
        temp << array[startIndex] << " " << arrayBackwardsAsString(array, startIndex-1, size);
    return temp.str();
}
